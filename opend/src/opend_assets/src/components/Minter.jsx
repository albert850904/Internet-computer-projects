...
function Minter() {
  ...
  const onSubmit = async (data) => {
    ...
    // data就是包含我們register的欄位的名字
    const name = data.name;
    const image = data.image[0];
    // arraybuffer 可以被用來把blob轉換成binary data
    const imageToByteData = [...new Uint8Array(await image.arrayBuffer())];

    const newNFTID = await opend.mint(imageToByteData, name);
